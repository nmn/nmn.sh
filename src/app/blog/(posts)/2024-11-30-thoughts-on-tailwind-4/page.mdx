...
Not having to manually configure `Autoprefixer` is a big win.
...
This is an unmitigated success and an obvious win. My biggest surprise was to learn that this wasn't
...
Tailwind 4 finally adds support for radial and conic gradients. While this is an obvious improvement,
...
The `*` variant for targeting direct children was already added and was problematic enough:
...
The Tailwind team tends to discourage the use of `tailwind-merge` and expect all styles to be
applied statically. However, IMO, this is an unrealistic expectation, as almost every serious
project that uses Tailwind, ususally also uses `tailwind-merge`.
...
_love_ this as this lets them maintain their previous proportions, but this is actually an
